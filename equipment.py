...
def generate_cyberware(npc: Npc, npc_template: NpcTemplate) -> Npc:
    ...
    def pick_cyberware(cyberware_name: str,
                       ...
                       remaining_humanity_budget: int) -> Tuple[bool, List[Item]]:
        nonlocal all_cyberware
        nonlocal npc

        if any(map(lambda x: x.name == cyberware_name, npc.cyberware)):
            return True, []

        cyperware_item: Item = next(cw for cw in all_cyberware if cw.name == cyberware_name)
        found_container: bool = len(cyperware_item.requires_container) == 0
        added_containers: List[Item] = list()
        for container in cyperware_item.requires_container:
            is_picked, added_containers = pick_cyberware(container, remaining_budget, remaining_humanity_budget)
            if is_picked:
                found_container = True
                remaining_budget -= sum([x.price for x in added_containers])
                remaining_humanity_budget -= sum([x.max_humanity_loss for x in added_containers])
                break

        if not found_container:
            return False, []

        if cyperware_item.price > remaining_budget:
            return False, []

        if cyperware_item.max_humanity_loss > remaining_humanity_budget:
            return False, []

        return True, added_containers + [cyperware_item]

    for _ in range(RANDOM_GENERATING_NUM_ATTEMPTS):
        chosen_cyberware: str = choose_exponential_random_element(npc_template.role.preferred_cyberware, True)
        is_picked, picked_cyberware = pick_cyberware(chosen_cyberware, cyberware_budget, humanity_budget)
        if is_picked:
            cyberware_budget -= sum([x.price for x in picked_cyberware])
            humanity_budget -= sum([x.max_humanity_loss for x in picked_cyberware])
